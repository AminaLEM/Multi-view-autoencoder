...
#                    batch_ys3 = test_output3[sample][:]
                    ...
                    feed_dictio={X1: batch_xs1,Y1:batch_ys1,}
...
#
    selected_features=np.genfromtxt('Selected_Features_NCI_GBM.csv', delimiter =',',skip_header =1)
    inputge=np.genfromtxt('mydatGE_GBM.csv',dtype  = np.unicode_, delimiter =',',skip_header =1)
#    inputcnv=np.genfromtxt('mydatME_GBM.csv',dtype  = np.unicode_, delimiter =',',skip_header =1)
#    inputmiRNA=np.genfromtxt('mydatMI_GBM.csv',dtype  = np.unicode_, delimiter =',',skip_header =1)
#
...
#            X_train3, X_test3 = model_selection.train_test_split(miRNA_sel_data, test_size=0.2, random_state=1)
            ...
            sae=   MutiViewAutoencoder(X_train1,X_test1, activation = act  )
            trials = Trials()
            #define the space of hyperparameters
            space = {
                     'units1': hp.choice('units1', range(2,n_hidden1)),
#                     'units2': hp.choice('units2', range(1,n_hidden2)),
#                     'units3': hp.choice('units3', range(1,n_hidden3)),
                     'batch_size': hp.choice('batch_size',[64,32,16,8,4] ) ,
...
#        train the HP optimization with 50 iterations
          best = fmin(sae.cross_validation, space, algo=tpe.suggest, max_evals=500,trials=trials)
          fname = './GBM_trials_HPMV8evals500_tanhGE.pkl'
...
#        save the best HPs
          ...
          f= open('./GBM_MVGBMparameter500_tanhGE.txt', 'a+')
