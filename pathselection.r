...
#miRNA features
miRNAcomponents = colnames(mydatMI)
miRNAcomponents=gsub("r","R",as.character(miRNAcomponents))
# mRNA features
genes= colnames(mydatGE)
...
for (i in  chosenpath) {
  ...
  print(length(path_sym2))
  path_sym2 = nodes(gsym)
  path_sym = c(path_sym,gsub("SYMBOL:", "", path_sym2))
  #get pathway genes as entrez IDs   
  gEntrez <- pathwayGraph(mapGeneEntrez@entries[[i]])
  path_Entrez2 = nodes(gEntrez)
  path_Entrez = c(path_Entrez,gsub("ENTREZID:", "", path_Entrez2))
...
path_sym= unique(path_sym)

methyd=unique(as.character(gene_comp[which(gene_comp[,1] %in%
                        path_sym),2]))
        
        #    
mirnad = unique(as.character(target[which(miRNA_target[,4] %in% path_Entrez)]))
                                          
mappingResge =  which(path_sym %in% colnames(mydatGE))
mappingResme= which(methyd %in% colnames(mydatME))
mappingResmi= which(mirnad %in% colnames(mydatMI))
write.csv(mappingResge,"E:/slectedGE.csv")
write.csv(mappingResme,"E:/slectedME.csv")
write.csv(mappingResmi,"E:/slectedMI.csv")

mapping_result= selected_features[complete.cases(selected_features), ]
write.csv(mapping_result, './data/selected_features.csv',row.names=FALSE)
